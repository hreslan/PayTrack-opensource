...
import { otherIncomeCsv } from "@/lib/csv";
...
export async function GET(req: Request) {
  ...
  const { otherIncome } = await loadExport(session.user.id, scope);

  const suffix = scopeFilenameSuffix(scope);
  return new Response(otherIncomeCsv(otherIncome), {
    headers: {
      "Content-Type": "text/csv; charset=utf-8",
      "Content-Disposition": `attachment; filename="other-income-${suffix}.csv"`,
